...
3. YAML разбирается рекурсивно: каждое конечное значение становится строкой листа,
   колонки «Уровень 1..N» — путь до значения (ключи словарей и номера элементов
   списков `[1]`, `[2]`, …). Ни один уровень вложенности не теряется.
